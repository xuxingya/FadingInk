    ...
    public class ToolManager
    {
        ...
        public Tool CreateTool(ToolType mode, Tool current_tool, bool toolIsPersistent)
        {
            ...
            switch (mode)
            {
                ...
                case ToolManager.ToolType.e_ink_create:
                    ...
                    //Console.WriteLine("freehand creat");
                    break;
        ...
        void mPDFView_TouchMove(object sender, System.Windows.Input.TouchEventArgs e)
        {
            ...
            if (!AlreadySwiped)
            {
                ...
                var Touch = e.GetTouchPoint(mPDFView);
                if (TouchIDs.Count == 1)
                {
                    //judge direction
                    if (Touch.Position.Y > TouchStart.Position.Y + 20 || Touch.Position.Y < TouchStart.Position.Y - 20)
                    {
                        updowndrag = true;
                    }
                    if (!updowndrag)
                    {
                        if (TouchStart != null && Touch.Position.X > (TouchStart.Position.X + 100) && !mPDFView.CanViewerScrollLeft())
                        {
                            if (mPDFView.GetDoc() != null)
                                mPDFView.GotoPreviousPage();
                            AlreadySwiped = true;
                            mPDFView.SetZoom(1.2, true);
                            //mPDFView.IsManipulationEnabled = false;
                            FreehandCreate.logtxt.Append("turn previous page " + "Time " + DateTime.Now.ToString() + "\r\n");
                        }

                        if (TouchStart != null && Touch.Position.X < (TouchStart.Position.X - 100) && !mPDFView.CanViewerScrollRight())
                        {
                            if (mPDFView.GetDoc() != null)
                            ...
                            mPDFView.SetZoom(1.2, true);
                            //mPDFView.IsManipulationEnabled = false;
                            FreehandCreate.logtxt.Append("turn next page " + "Time " + DateTime.Now.ToString() + "\r\n");
                        }
                    }
                }
